...
```
The top-ranked differentially expressed genes identified across the different comparisons show large log2 fold changes together with strong statistical significance, indicating substantial differences in gene expression between the analyzed conditions.  
The presence of both moderate and extremely high fold changes suggests that some genes undergo particularly pronounced transcriptional alterations, potentially reflecting distinct molecular mechanisms associated with tumor development and regional heterogeneity.  
Adjusted p-values were omitted from the final tables for visualization purposes, as several genes displayed extremely low values (in some cases reaching magnitudes close to \(10^{-192}\)), making the tables less readable while providing limited additional interpretative value.
